...
public class Board
{
    ...
    internal void RemoveItem(Cell cell)
    {
        cell.Free();
    }
    ...
    internal void Fill()
    {
        List<NormalItem.eNormalType> pool = BuildPool();
    ...
    private List<NormalItem.eNormalType> BuildPool()
    {
        int groups = (boardSizeX * boardSizeY) / 3;

        var allTypes = System.Enum.GetValues(typeof(NormalItem.eNormalType));

        List<NormalItem.eNormalType> pool = new List<NormalItem.eNormalType>(groups * 3);

        foreach (NormalItem.eNormalType type in allTypes)
        {
            if (pool.Count / 3 >= groups) break;
            pool.Add(type);
            pool.Add(type);
            pool.Add(type);
        }

        int remaining = groups - (pool.Count / 3);
        ...
        for (int i = pool.Count - 1; i > 0; i--)
        {
            ...
            NormalItem.eNormalType tmp = pool[i];
            pool[i] = pool[j];
            pool[j] = tmp;
        }
