...
public class BoardController : MonoBehaviour
{
    ...
    public void StartGame(GameManager gameManager, GameSettings gameSettings, GameManager.eLevelMode levelMode)
    {
        ...
        m_tray = new Tray(this.transform, gameSettings);
    ...
    public void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (hit.collider != null)
            {
                TrayItemView trayItem = hit.collider.GetComponent<TrayItemView>();
                if (trayItem != null && m_levelMode == GameManager.eLevelMode.TIMER)
                {
                    HandleTrayTap(trayItem);
                    return;
                }
    ...
    private void HandleTap(Cell cell)
    {
        if (cell == null || cell.Item == null) return;

        if (m_tray.IsFull && m_levelMode == GameManager.eLevelMode.MOVES)
        {
            m_gameManager.GameOver();
            return;
        }

        if (m_tray.IsFull && m_levelMode == GameManager.eLevelMode.TIMER) return;

        IsBusy = true;

        m_tray.AddItem(cell.Item);
        m_board.RemoveItem(cell);

        StartCoroutine(AfterTapCoroutine());

        OnMoveEvent.Invoke();
    }

    private IEnumerator AfterTapCoroutine()
    {
    ...
    private void HandleTrayTap(TrayItemView trayItem)
    {
        ...
        Item item = m_tray.ReturnItem(trayItem);
        if (item == null)
        {
            IsBusy = false;
            return;
